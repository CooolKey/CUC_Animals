// pages/animalGallery/animalGallery.js
...
Page({
  ...
  likePhoto({currentTarget}){
    let {idx, liked} = currentTarget.dataset;
    if(liked) return;
    let photos = this.data.photos;
    photos[idx].like += 1;
    photos[idx].liked = true;
    this.setData({
      photos: photos
    })
  },
  ...
  onLoad: function (options) {
    ...
    this.data.name=name
    const _ = db.command
    db.collection('photos').where({name:name}).count({
      success:res =>{
    ...
    db.collection('photos').where({name:name}
    ).get({
      success:res =>{
        this.setData({
          photos:res.data,
  ...
  onReachBottom: function () {
    ...
    if (this.data.photo_cnt <= this.data.photo_now) {
      ...
      wx.showToast({
        ...
        db.collection('photos').where({name:name}).skip(this.data.photo_now).get()
        .then(res => {
          //console.log(res);
